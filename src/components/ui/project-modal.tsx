...
export default function ProjectModal({ project, onOpen, onClose }: ProjectModalProps) {
	...
	return (
		<Dialog open={onOpen} onClose={onClose} className="relative z-[100]">
			...
			<div
				data-lenis-prevent
				className="fixed inset-0 z-10 w-screen overflow-y-auto overscroll-contain"
			>
				<div className="flex min-h-full items-end justify-center p-0 sm:items-center sm:p-6 lg:p-8">
					<DialogPanel
						transition
						className="relative flex max-h-[92dvh] w-full max-w-5xl transform flex-col overflow-hidden rounded-t-[2rem] bg-[#101010] text-left shadow-[0_40px_120px_rgba(0,0,0,0.65)] transition-all duration-300 ease-out data-closed:translate-y-8 data-closed:opacity-0 sm:max-h-[min(90dvh,56rem)] sm:rounded-[2rem] sm:data-closed:translate-y-4 sm:data-closed:scale-[0.98]"
					>
						<button
							type="button"
							onClick={onClose}
						...
						>
							<div className="grid grid-cols-1 lg:grid-cols-2">
								...
								<div className="flex flex-col justify-between gap-8 px-6 pb-[max(2rem,env(safe-area-inset-bottom))] pt-6 sm:px-8 sm:pb-10 sm:pt-8 lg:px-10 lg:py-10">
